...
export function AdminLayout({ children, title }: AdminLayoutProps) {
  ...
  const matchedProducts = queryLower
    ? (products ?? []).filter((p) => p.name.toLowerCase().includes(queryLower) || p.brandName?.toLowerCase().includes(queryLower) || p.sku?.toLowerCase().includes(queryLower)).slice(0, 5)
    : [];
                            ...
                            {matchedProducts.map((p) => (
                              ...
                              >
                                <div className={"flex items-center justify-between"}>
                                  ...
                                  <span className={"text-xs font-bold text-[#2C5F2D]"}>KSh {(p.sellingPrice || 0).toFixed(2)}</span>
                                </div>
