          ...
          <div className="space-y-1 max-h-60 overflow-y-auto">
            {results.map((c) => (
              <button
                ...
                className="w-full text-left px-3 py-2.5 rounded-md hover:bg-[#F7F3E9] border border-[#E3DCC8] transition-colors"
              >
                <p className="text-sm font-medium text-[#2C5F2D]">{c.name}</p>
                <p className="text-sm text-[#5C6B5C]">{c.phone} · {c.loyaltyPoints} pts</p>
              </button>
            ))}
            {debouncedSearch.length >= 2 && results.length === 0 && (
              <p className="text-sm text-[#5C6B5C] text-center py-4">No customers found.</p>
            )}
          </div>
