...
export function PaymentModal({
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`Payment — ${formatCurrency(grandTotal)}`} maxWidth="md">
      {/* Tabs */}
      <div className="flex border-b border-[#E3DCC8] mb-4 -mx-6 px-6">
        {tabs.map((tab) => (
          <button
            key={tab.key}
            ...
            className={`flex items-center gap-1.5 px-4 py-2.5 text-sm font-medium border-b-2 -mb-px transition-colors ${
              method === tab.key
                ...
                : "border-transparent text-[#5C6B5C] hover:text-[#2C5F2D]"
            }`}
          >
            <tab.icon size={14} />
            {tab.label}
      ...
      {method === "cash" && (
        <div className="space-y-4">
          <div className="bg-[#F7F3E9] rounded-md p-4 text-center">
            <p className="text-sm text-[#5C6B5C] mb-1">Amount Due</p>
      ...
      {method === "mpesa" && (
        <div className="space-y-4">
          <div className="bg-[#F7F3E9] rounded-md p-4 text-center">
            <p className="text-sm text-[#5C6B5C] mb-1">Amount to Charge</p>
            <p className="text-3xl font-semibold font-mono tabular-nums">{formatCurrency(grandTotal)}</p>
          </div>
          ...
          {(mpesaStatus === "processing" || mpesaStatus === "polling") && (
            <div className="text-center py-6 space-y-3">
              ...
              <p className="text-sm font-medium text-[#2C5F2D]">Waiting for M-Pesa confirmation...</p>
              <p className="text-sm text-[#5C6B5C]">Ask customer to enter their M-Pesa PIN</p>
              <p className="text-sm text-[#5C6B5C]">Phone: {mpesaPhone}</p>
            </div>
          )}
          ...
          {mpesaStatus === "success" && (
            <div className="space-y-4">
              <div className="text-center py-4 space-y-2">
                ...
                <p className="text-sm font-medium text-[#2C5F2D]">M-Pesa payment confirmed!</p>
                <p className="text-sm text-[#5C6B5C] font-mono">{mpesaRef}</p>
              </div>
          ...
          {mpesaStatus === "failed" && (
            <div className="space-y-4">
              <div className="text-center py-4 space-y-2">
                ...
                <p className="text-sm font-medium text-[#2C5F2D]">Payment failed</p>
                <p className="text-sm text-[#5C6B5C]">Ask customer to check their M-Pesa and try again</p>
              </div>
          ...
          {(mpesaStatus === "idle" || mpesaStatus === "success") && (
            <div className="border-t border-[#E3DCC8] pt-3">
              <p className="text-sm text-[#5C6B5C] mb-2">Or enter M-Pesa code manually:</p>
      ...
      {method === "split" && (
        <div className="space-y-4">
          <div className="bg-[#F7F3E9] rounded-md p-4 text-center">
            <p className="text-sm text-[#5C6B5C] mb-1">Total Amount</p>
            <p className="text-3xl font-semibold font-mono tabular-nums">{formatCurrency(grandTotal)}</p>
          </div>
