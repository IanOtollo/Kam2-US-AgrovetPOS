...
function ProductRowSkeleton() {
  return (
    <div className="flex items-center gap-3 p-2.5 rounded-md border border-[#E3DCC8] bg-white">
...
export function ProductGrid({ onAddItem, searchRef }: ProductGridProps) {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Filters */}
      <div className="p-3 border-b border-[#E3DCC8] space-y-2 flex-shrink-0">
        <SearchInput
          ref={resolvedRef}
          ...
          <button
            ...
            className={cn(
              ...
              selectedCategory === "all"
                ...
                : "bg-white text-[#5C6B5C] border-[#E3DCC8] hover:border-[#2C5F2D]"
            )}
          >
            All
          </button>
          ...
          <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
            <button
              ...
              className={cn(
                ...
                selectedBrand === "all"
                  ? "bg-[#E3DCC8] text-[#2C5F2D] border-[#E3DCC8] font-medium"
                  : "bg-white text-[#5C6B5C] border-[#E3DCC8] hover:text-[#2C5F2D]"
              )}
            >
              All Brands
            </button>
              ...
              <button
                ...
                className={cn(
                  ...
                  selectedBrand === brand._id
                    ? "bg-[#E3DCC8] text-[#2C5F2D] border-[#2C5F2D] font-medium"
                    : "bg-white text-[#5C6B5C] border-[#E3DCC8] hover:text-[#2C5F2D]"
                )}
              >
                {brand.name}
      ...
      <div className="flex-1 overflow-y-auto p-3 pb-24 md:pb-3">
        {products === undefined ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-center">
            <Package size={32} className="text-[#5C6B5C] mb-2" strokeWidth={1.5} />
            <p className="text-sm text-[#5C6B5C]">No products found.</p>
          </div>
        ) : (
          <div className="flex flex-col gap-1.5">
                ...
                <div key={product._id} className="flex flex-col">
                  <button
                    ...
                    className={cn(
                      ...
                      isOutOfStock
                        ? "opacity-40 cursor-not-allowed bg-[#F7F3E9] border-[#E3DCC8]"
                        : "bg-white border-[#E3DCC8] hover:border-[#2C5F2D] hover:shadow-sm active:scale-[0.99]"
                    )}
                  >
                    {/* Image / placeholder */}
                    <div className="w-14 h-14 sm:w-16 sm:h-16 flex-shrink-0 rounded-md bg-[#F7F3E9] flex items-center justify-center overflow-hidden">
                      {product.imageUrl
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[11px] text-[#5C6B5C] leading-none mb-0.5 truncate">{product.brandName}</p>
                      <p className="text-sm font-medium text-[#2C5F2D] leading-tight truncate">{product.name}</p>
                    ...
                    <div className="flex-shrink-0 text-right">
                      <p className="text-sm font-semibold font-mono tabular-nums text-[#2C5F2D]">
                  ...
                  {isExpanded && activeVariants.length > 1 && (
                    <div className="mt-1 bg-white border border-[#2C5F2D] rounded-md p-2 space-y-1 shadow-sm">
                      <p className="text-xs font-medium text-[#5C6B5C] px-1 pb-1">Select size:</p>
