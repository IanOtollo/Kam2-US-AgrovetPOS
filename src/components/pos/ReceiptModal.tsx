...
export function ReceiptModal({ isOpen, saleId, onNewSale }: ReceiptModalProps) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onNewSale} title="Sale Complete" maxWidth="sm">
      ...
      {/* Printable receipt */}
      <div className="receipt-printable border border-[#E3DCC8] rounded-md p-4 font-mono text-sm text-[#2C5F2D]">
        ...
        <div className="border-t border-b border-dashed border-[#5C6B5C] py-2 mb-2">
          <div className="flex justify-between">
            <span>Date: {formatDate(sale.createdAt)}</span>
            <span>Time: {formatTime(sale.createdAt)}</span>
          </div>
          <p>Attendant: {sale.cashierName}</p>
        ...
        <div className="border-t border-dashed border-[#5C6B5C] pt-2 space-y-0.5">
          <div className="flex justify-between">
            <span>Subtotal</span>
          ...
          <div className="flex justify-between font-bold text-sm pt-1 border-t border-dashed border-[#5C6B5C]">
            <span>TOTAL</span>
            <span>{formatCurrency(sale.grandTotal)}</span>
          </div>
        </div>

        <div className="border-t border-dashed border-[#5C6B5C] mt-2 pt-2 space-y-0.5">
        ...
        <div className="text-center mt-3 border-t border-dashed border-[#5C6B5C] pt-2">
          <p>{receiptFooter}</p>
        </div>
      </div>
    </Modal>
