...
export function SkeletonTable({ rows = 5, cols = 4 }: { rows?: number; cols?: number }) {
  return (
    <div className="bg-white border border-[#E3DCC8] rounded-md overflow-hidden">
      <div className="h-10 bg-[#E3DCC8] px-4 flex items-center gap-4">
        {Array.from({ length: cols }).map((_, i) => (
          <Skeleton key={i} className="h-3 flex-1 bg-[#D8D8D8]" />
        ))}
      </div>
      {Array.from({ length: rows }).map((_, i) => (
        <div key={i} className="h-12 border-b border-[#E3DCC8] px-4 flex items-center gap-4">
...
export function SkeletonCard({ className }: { className?: string }) {
  return (
    <div className={cn("bg-white border border-[#E3DCC8] rounded-md p-4 space-y-3", className)}>
