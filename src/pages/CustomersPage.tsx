      ...
      {!allCustomers ? (
        ...
        <>
          ...
          <div className="md:hidden space-y-3">
            {displayCustomers.map((c) => (
              <div key={c._id} className="bg-white border border-[#E3DCC8] rounded-md p-4">
                <div>
                  <p className="font-medium text-sm">{c.name}</p>
                  <p className="text-xs text-[#5C6B5C] mt-0.5">{c.phone}</p>
                </div>
                <div className="grid grid-cols-3 gap-2 mt-3 pt-3 border-t border-[#E3DCC8] text-center text-xs">
                  <div>
                    <p className="text-[#5C6B5C]">Spent</p>
                    <p className="font-mono font-semibold text-sm">{formatCurrency(c.totalSpent)}</p>
                  </div>
                  <div>
                    <p className="text-[#5C6B5C]">Visits</p>
                    <p className="font-semibold text-sm">{c.visitCount}</p>
                  </div>
                  <div>
                    <p className="text-[#5C6B5C]">Points</p>
                    <p className="font-semibold text-sm">{c.loyaltyPoints}</p>
                  </div>
                </div>
                <div className="flex gap-2 mt-3 pt-3 border-t border-[#E3DCC8]">
                  <button onClick={() => openEdit(c)} className="flex-1 text-sm text-[#2563EB] py-1.5 border border-[#E3DCC8] rounded-md">Edit</button>
                  <button onClick={() => setProfileId(c._id)} className="flex-1 text-sm text-[#5C6B5C] py-1.5 border border-[#E3DCC8] rounded-md">View Profile</button>
                </div>
              </div>
            ))}
          </div>
        ...
        {profileCustomer && (
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-[#F7F3E9] rounded-md p-3 text-center">
                <p className="text-sm text-[#5C6B5C]">Total Spent</p>
                <p className="text-lg font-mono font-semibold">{formatCurrency(profileCustomer.totalSpent)}</p>
              </div>
              <div className="bg-[#F7F3E9] rounded-md p-3 text-center">
                <p className="text-sm text-[#5C6B5C]">Visits</p>
                <p className="text-lg font-semibold">{profileCustomer.visitCount}</p>
              </div>
              <div className="bg-[#F7F3E9] rounded-md p-3 text-center">
                <p className="text-sm text-[#5C6B5C]">Loyalty Points</p>
                <p className="text-lg font-semibold">{profileCustomer.loyaltyPoints}</p>
              </div>
            </div>
            <div>
              <p className="text-sm font-medium uppercase tracking-wider text-[#5C6B5C] mb-2">Purchase History</p>
              {(customerSales ?? []).length === 0 ? (
                <p className="text-sm text-[#5C6B5C]">No purchases yet.</p>
              ) : (
                <div className="space-y-1 max-h-60 overflow-y-auto">
                  {(customerSales ?? []).map((s) => (
                    <div key={s._id} className="flex justify-between items-center py-2 border-b border-[#E3DCC8] last:border-0 text-sm">
                      <div>
                        <p className="font-mono text-sm">{s.saleNumber}</p>
                        <p className="text-sm text-[#5C6B5C]">{formatDate(s.createdAt)}</p>
                      </div>
