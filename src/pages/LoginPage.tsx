...
export function LoginPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F7F3E9] flex items-center justify-center p-4">
      <div className="bg-white border border-[#E3DCC8] rounded-md p-8 w-full max-w-sm shadow-sm">
        <div className="text-center mb-8">
          <div className="w-24 h-24 mx-auto mb-4 rounded-full bg-white border-2 border-[#E3DCC8] shadow-md overflow-hidden flex items-center justify-center">
            <img
              src="/kam2-us-logo.png"
              alt="Kam2-US Agrovet Bungengi Logo"
              className="w-full h-full object-contain"
            />
          </div>
          <h1 className="text-xl font-semibold text-[#2C5F2D] tracking-tight">{shopName}</h1>
          <p className="text-sm text-[#5C6B5C] mt-1">Enter your PIN to continue</p>
        </div>
        ...
        {isLocked ? (
          <div className="mb-6 rounded-md bg-red-50 border border-red-200 p-4 text-center">
            ...
            <p className="text-xs text-[#5C6B5C] mt-1">Too many failed attempts</p>
            <p className="text-2xl font-mono font-bold text-[#DC2626] mt-2">{formatCountdown(countdown)}</p>
            <p className="text-xs text-[#5C6B5C] mt-1">Unlocks automatically</p>
          </div>
        ) : (
          <>
            {/* PIN dots */}
              ...
              {[0, 1, 2, 3].map((i) => (
                <div
                  ...
                  className={cn(
                    ...
                    pin.length > i ? "bg-[#2C5F2D] border-[#2C5F2D]" : "bg-transparent border-[#E3DCC8]"
                  )}
                />
              ))}
            </div>
              ...
              {digits.map((d, i) => {
                ...
                if (d === "del") {
                  return (
                    <button
                      ...
                      className="h-14 rounded-md flex items-center justify-center text-[#5C6B5C] hover:bg-[#F7F3E9] active:bg-[#E3DCC8] transition-colors duration-100 disabled:opacity-30"
                    >
                ...
                return (
                  <button
                    ...
                    className="h-14 rounded-md text-xl font-medium text-[#2C5F2D] hover:bg-[#F7F3E9] active:bg-[#E3DCC8] border border-[#E3DCC8] transition-colors duration-100 disabled:opacity-30"
                  >
                    {d}
                  </button>
                );
            ...
            {loading && <p className="text-sm text-[#5C6B5C] text-center mt-4">Verifying...</p>}
          </>
        )}

        <p className="text-center mt-6" style={{ fontFamily: "'Dancing Script', cursive", fontSize: "20px", color: "#2C5F2D", letterSpacing: "0.02em" }}>
          Kam2-US Agrovet
        </p>
      </div>
    </div>
