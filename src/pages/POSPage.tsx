...
export function POSPage() {
  ...
  return (
    <div className="flex flex-col h-screen bg-white overflow-hidden pb-16 md:pb-20">
      {/* POS Header */}
      <header className="sticky top-0 z-50 h-14 bg-[#2C5F2D] shadow-md text-white flex items-center px-4 gap-3 flex-shrink-0">
        {/* Brand */}
        ...
        {user?.role === "admin" && (
          <button
            onClick={() => navigate("/")}
            className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-white/10 hover:bg-white/20 text-sm text-white transition-colors"
          >
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        {mobileTab === "products" && (
          <button
            ...
            className="md:hidden fixed bottom-20 right-4 z-30 flex items-center gap-2 bg-[#2C5F2D] text-white pl-4 pr-5 py-3 rounded-full shadow-lg text-sm font-semibold"
          >
      ...
      {showHeldSales && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-md border border-[#E3DCC8] w-full max-w-md p-6 shadow-sm">
            ...
            <div className="space-y-2 max-h-80 overflow-y-auto">
              {(heldSales ?? []).map((sale) => (
                <button
                  ...
                  className="w-full text-left p-3 border border-[#E3DCC8] rounded-md hover:bg-[#F7F3E9] transition-colors"
                >
                  <div className="flex justify-between items-start">
                    <div>
                      <p className="text-sm font-medium">{sale.saleNumber}</p>
                      <p className="text-sm text-[#5C6B5C]">{formatDateTime(sale.createdAt)}</p>
                      <p className="text-sm text-[#5C6B5C]">{sale.items.length} items</p>
                    </div>
            ...
            <button
              ...
              className="mt-4 w-full h-10 border border-[#E3DCC8] rounded-md text-sm text-[#5C6B5C] hover:bg-[#F7F3E9] transition-colors"
            >
              Close
            </button>
          </div>
