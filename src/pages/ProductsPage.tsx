...
const CSV_TEMPLATE_HEADERS = "name,brand,category,description,sku,packageSize,costPrice,sellingPrice,stockQuantity,lowStockThreshold,barcode,expiryDate";
...
function parseCSV(text: string): { rows: ImportRow[]; errors: string[] } {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const packageSizeVal = parseFloat(get("packageSize") || get("packageSize")) || 0;
    if (!name || !brand || !category) {
      errors.push(`Row ${i + 1}: name, brand, and category are required.`);
      continue;
    }
    if (packageSizeVal <= 0) {
      errors.push(`Row ${i + 1}: packageSize must be a positive number.`);
      continue;
    }
    ...
    try {
      if (editingId) {
        await updateProduct({
          ...
          brandId: productForm.brandId ? (productForm.brandId as Id<"brands">) : undefined,
      ...
      {!products ? (
        ...
        <>
                    ...
                    <TableRow>
                      <TableCell>
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            className="text-[#5C6B5C] hover:text-[#2C5F2D] transition-colors"
                          >
                            {expandedProduct === product._id ? <ChevronDown size={14} /> : <ChevronRight size={14} />}
                          </button>
                          <span className="font-medium text-[#2C5F2D]">{product.name}</span>
                        </div>
                      </TableCell>
                      ...
                      <TableCell align="right">
                        <div className="flex items-center justify-end gap-1">
                          <button onClick={() => openEdit(product)} className="btn-icon hover:bg-[#E3DCC8]">
                            <Edit size={14} />
                          </button>
                          <button
                      ...
                      <tr className="bg-[#F7F3E9]">
                        <td colSpan={6} className="px-8 py-3">
                          <div className="space-y-1">
                            <p className="text-sm font-medium uppercase tracking-wider text-[#5C6B5C] mb-2">Variants</p>
                            {product.variants.map((v) => (
          ...
          <div className="md:hidden space-y-3">
            {filtered.map((product) => (
              <div key={product._id} className="bg-white border border-[#E3DCC8] rounded-md p-4">
                <div className="flex items-start gap-3">
                  {product.imageUrl ? (
                    <img src={product.imageUrl} alt={product.name} className="w-14 h-14 rounded object-cover flex-shrink-0 bg-[#F7F3E9]" />
                  ) : (
                    <div className="w-14 h-14 rounded bg-[#F7F3E9] flex-shrink-0 flex items-center justify-center">
                      <span className="text-xs text-[#5C6B5C]">IMG</span>
                    </div>
                  )}
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-sm text-[#2C5F2D] truncate">{product.name}</p>
                    <p className="text-xs text-[#5C6B5C]">{product.brandName} · {product.categoryName}</p>
                ...
                <div className="flex gap-2 mt-3 pt-3 border-t border-[#E3DCC8]">
                  <button onClick={() => openEdit(product)} className="flex-1 text-sm text-[#2563EB] py-1.5 border border-[#E3DCC8] rounded-md">Edit</button>
                  <button onClick={() => setDeleteId(product._id)} className="flex-1 text-sm text-[#DC2626] py-1.5 border border-[#E3DCC8] rounded-md">Delete</button>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div>
              <div className="flex items-center gap-1 mb-1">
                <label className="text-sm font-medium text-[#2C5F2D]">Brand (Optional)</label>
                {!showAddBrand && (
                  <button
                    type="button"
                    onClick={() => { setShowAddBrand(true); setNewBrandName(""); }}
                    className="ml-auto flex items-center gap-1 text-sm text-[#2C5F2D] hover:text-[#1B3E1C] font-medium"
                    title="Add new brand"
                  >
                    <Plus size={13} /> New
                  </button>
              ...
              {showAddBrand ? (
                ...
                <Select options={[{ value: "", label: "No Brand" }, ...brandOptions]} value={productForm.brandId} onChange={(e) => setProductForm({ ...productForm, brandId: e.target.value })} placeholder="Select brand" />
              )}
            </div>
            ...
            <div>
              <div className="flex items-center gap-1 mb-1">
                <label className="text-sm font-medium text-[#2C5F2D]">Category *</label>
                {!showAddCategory && (
                  <button
                    type="button"
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <p className="text-sm font-medium uppercase tracking-wider text-[#5C6B5C]">Variants</p>
            ...
            <div className="space-y-3 max-h-80 overflow-y-auto pr-1">
              {variants.map((v, i) => (
                <div key={i} className="border border-[#E3DCC8] rounded-md p-3 space-y-2">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-sm font-medium text-[#5C6B5C]">Variant {i + 1}</span>
                    {variants.length > 1 && (
      ...
      <Modal isOpen={showImportModal} onClose={() => setShowImportModal(false)} title="Import Products from CSV" maxWidth="lg">
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <p className="text-sm text-[#5C6B5C]">Upload a CSV file to bulk-create products and variants.</p>
            <button
              onClick={downloadTemplate}
              className="flex items-center gap-1 text-sm text-[#2C5F2D] hover:underline font-medium"
            >
              <Download size={14} /> Download Template
            </button>
          </div>

          <div className="border-2 border-dashed border-[#E3DCC8] rounded-md p-6 text-center">
            <Upload size={24} className="mx-auto text-[#5C6B5C] mb-2" />
            <p className="text-sm text-[#5C6B5C] mb-2">Select a CSV file to import</p>
          ...
          <div className="bg-[#F7F3E9] rounded-md p-3 text-xs text-[#5C6B5C]">
            <p className="font-medium mb-1">Required columns: <span className="font-mono">name, brand, category, packageSize, sellingPrice</span></p>
          ...
          {importRows.length > 0 && (
            <div>
              <p className="text-sm font-medium text-[#5C6B5C] mb-2">{importRows.length} row{importRows.length !== 1 ? "s" : ""} ready to import</p>
              <div className="border border-[#E3DCC8] rounded-md overflow-x-auto max-h-48">
                <table className="w-full text-xs">
                  ...
                  <tbody>
                    {importRows.slice(0, 50).map((r, i) => (
                      <tr key={i} className="border-t border-[#E3DCC8]">
                        <td className="px-3 py-1.5 font-medium">{r.name}</td>
                        <td className="px-3 py-1.5">{r.brand}</td>
                        <td className="px-3 py-1.5">{r.category}</td>
                        <td className="px-3 py-1.5">{r.packageSize}</td>
                ...
                {importRows.length > 50 && (
                  <p className="text-xs text-[#5C6B5C] px-3 py-2">…and {importRows.length - 50} more rows</p>
                )}
              </div>
            </div>
          )}
