  ...
  return (
    <div className="flex flex-col sm:flex-row sm:flex-wrap sm:items-center gap-2 mb-6">
      <div className="flex items-center gap-2 flex-1">
        ...
        <span className="text-[#5C6B5C] flex-shrink-0">to</span>
...
function ProductReport() {
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <p className="text-sm font-medium uppercase tracking-wider text-[#5C6B5C] mb-3">Top 10 by Revenue</p>
          <div className="bg-white border border-[#E3DCC8] rounded-md overflow-hidden">
            <Table>
              ...
              <TableBody>
                {data.topByRevenue.map((p, i) => (
                  <TableRow key={p.sku}>
                    <TableCell><span className="text-[#5C6B5C] mr-2">{i + 1}.</span>{p.name}</TableCell>
        ...
        <div>
          <p className="text-sm font-medium uppercase tracking-wider text-[#5C6B5C] mb-3">Top 10 by Quantity</p>
          <div className="bg-white border border-[#E3DCC8] rounded-md overflow-hidden">
            <Table>
...
function ProfitReport() {
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-white border border-[#E3DCC8] rounded-md p-4">
          <p className="text-sm font-medium uppercase tracking-wider text-[#5C6B5C] mb-3">P&L Summary</p>
          <div className="space-y-2 text-sm">
            <div className="flex justify-between"><span className="text-[#5C6B5C]">Revenue</span><span className="font-mono">{formatCurrency(data.totalRevenue)}</span></div>
            <div className="flex justify-between"><span className="text-[#5C6B5C]">Cost of Goods</span><span className="font-mono text-[#DC2626]">-{formatCurrency(data.totalCOGS)}</span></div>
            <div className="flex justify-between font-medium pt-1 border-t border-[#E3DCC8]"><span>Gross Profit</span><span className="font-mono">{formatCurrency(data.grossProfit)}</span></div>
            <div className="flex justify-between"><span className="text-[#5C6B5C]">Expenses</span><span className="font-mono text-[#DC2626]">-{formatCurrency(data.totalExpenses)}</span></div>
            <div className="flex justify-between font-semibold text-sm pt-1 border-t border-[#E3DCC8]"><span>Net Profit</span><span className={`font-mono ${data.netProfit >= 0 ? "text-[#16A34A]" : "text-[#DC2626]"}`}>{formatCurrency(data.netProfit)}</span></div>
          </div>
        </div>
        <div className="bg-white border border-[#E3DCC8] rounded-md p-4">
          <p className="text-sm font-medium uppercase tracking-wider text-[#5C6B5C] mb-3">Expenses by Category</p>
          <div className="space-y-2 text-sm">
            {Object.entries(data.expensesByCategory).map(([cat, amount]) => (
              <div key={cat} className="flex justify-between">
                <span className="capitalize text-[#5C6B5C]">{cat}</span>
                <span className="font-mono">{formatCurrency(amount)}</span>
              </div>
            ))}
            {Object.keys(data.expensesByCategory).length === 0 && <p className="text-[#5C6B5C]">No expenses in this period.</p>}
          </div>
        </div>
      </div>
    </div>
...
function PaymentReport() {
  ...
  const COLORS = ["#2C5F2D", "#16A34A", "#2563EB", "#D97706"];
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-white border border-[#E3DCC8] rounded-md p-4">
          <p className="text-sm font-medium uppercase tracking-wider text-[#5C6B5C] mb-4">Revenue by Payment Method</p>
          <div className="space-y-3">
            {chartData.map(({ name, value }, i) => (
              <div key={name}>
                ...
                <div className="h-2 bg-[#E3DCC8] rounded-full">
  ...
  return (
    <div>
      ...
      {data.length === 0 ? (
        ...
        <>
          <div className="bg-white border border-[#E3DCC8] rounded-md overflow-hidden mb-4">
            <Table>
              <TableHead>
                <tr>
          ...
          <div className="bg-white border border-[#E3DCC8] rounded-md p-4">
            <p className="text-sm font-medium uppercase tracking-wider text-[#5C6B5C] mb-4">Revenue by Staff</p>
            <ResponsiveContainer width="100%" height={180}>
              <BarChart data={data} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#E3DCC8" />
                <XAxis dataKey="name" tick={{ fontSize: 11, fill: "#5C6B5C" }} />
                <YAxis tick={{ fontSize: 11, fill: "#5C6B5C" }} tickFormatter={(v) => `${(v / 1000).toFixed(0)}k`} />
                <Tooltip formatter={(v) => formatCurrency(Number(v))} contentStyle={{ fontSize: 12, border: "1px solid #E3DCC8", borderRadius: 4 }} />
                <Bar dataKey="revenue" fill="#2C5F2D" radius={[2, 2, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </>
