  ...
  return (
    <AdminLayout title="Sales History">
      ...
      <div className="flex flex-col sm:flex-row sm:flex-wrap sm:items-center gap-2 mb-4">
        ...
        <span className="text-sm text-[#5C6B5C]">{filtered.length} results</span>
      </div>
      ...
      {!salesQuery ? (
        ...
        <>
          ...
          <div className="md:hidden space-y-3">
            {filtered.map((sale) => (
              <div key={sale._id} className="bg-white border border-[#E3DCC8] rounded-md p-4" onClick={() => setSelectedSale(sale._id)}>
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <p className="font-mono text-sm font-semibold">{sale.saleNumber}</p>
                    <p className="text-xs text-[#5C6B5C] mt-0.5">{formatDateTime(sale.createdAt)}</p>
                  </div>
                  <StatusBadge status={sale.status} />
                </div>
                <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 mt-3 pt-3 border-t border-[#E3DCC8] text-sm">
                  <div>
                    <p className="text-xs text-[#5C6B5C]">Items</p>
                    <p>{sale.items.reduce((s, i) => s + i.quantity, 0)}</p>
                  </div>
                  <div>
                    <p className="text-xs text-[#5C6B5C]">Payment</p>
                    <p className="capitalize">{methodLabel[sale.paymentMethod] ?? sale.paymentMethod}</p>
                  </div>
                  <div className="col-span-2">
                    <p className="text-xs text-[#5C6B5C]">Total</p>
                ...
                <div className="flex gap-2 mt-3 pt-3 border-t border-[#E3DCC8]">
                  <button className="flex-1 text-sm text-[#2C5F2D] py-1.5 border border-[#E3DCC8] rounded-md" onClick={(e) => { e.stopPropagation(); setSelectedSale(sale._id); }}>
                    View
                  </button>
                  {isAdmin && (
                    <button className="flex-1 text-sm text-[#DC2626] py-1.5 border border-[#E3DCC8] rounded-md" onClick={(e) => { e.stopPropagation(); setDeleteId(sale._id); }}>
                      Delete
                    </button>
                  )}
                </div>
        ...
        {saleDetail && (
          <div className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
              <div>
                <p className="text-xs text-[#5C6B5C]">Sale Number</p>
                <p className="font-mono font-medium">{saleDetail.saleNumber}</p>
              </div>
              <div>
                <p className="text-xs text-[#5C6B5C]">Date & Time</p>
                <p>{formatDateTime(saleDetail.createdAt)}</p>
              </div>
              <div>
                <p className="text-xs text-[#5C6B5C]">Attendant</p>
                <p>{saleDetail.cashierName}</p>
              </div>
              <div>
                <p className="text-xs text-[#5C6B5C]">Customer</p>
                <p>{saleDetail.customerName}</p>
              </div>
            </div>

            {/* Desktop items table */}
            <div className="hidden sm:block border border-[#E3DCC8] rounded-md overflow-hidden">
              <Table>
                <TableHead>
                  <tr>
            ...
            <div className="bg-[#F7F3E9] rounded-md p-3 space-y-1 text-sm">
              <div className="flex justify-between"><span className="text-[#5C6B5C]">Subtotal</span><span className="font-mono">{formatCurrency(saleDetail.subtotal)}</span></div>
              ...
              <div className="flex justify-between"><span className="text-[#5C6B5C]">VAT</span><span className="font-mono">{formatCurrency(saleDetail.taxAmount)}</span></div>
              <div className="flex justify-between font-semibold text-sm pt-1 border-t border-[#E3DCC8]"><span>Grand Total</span><span className="font-mono">{formatCurrency(saleDetail.grandTotal)}</span></div>
            </div>
          </div>
        )}
      </Modal>
