  ...
  return (
    <div className="space-y-4 w-full h-full">
      ...
      {!orders ? (
        ...
        <>
          ...
          <div className="md:hidden space-y-3">
            {orders.map((o) => (
              <div key={o._id} className="bg-white border border-[#E3DCC8] rounded-md p-4">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <p className="font-mono text-sm font-semibold">{o.poNumber}</p>
                    <p className="text-xs text-[#5C6B5C] mt-0.5">{o.supplierName}</p>
                  </div>
                  <StatusBadge status={o.status} />
                </div>
                <div className="grid grid-cols-3 gap-2 mt-3 pt-3 border-t border-[#E3DCC8] text-xs">
                  <div>
                    <p className="text-[#5C6B5C]">Items</p>
                    <p className="font-medium text-sm">{o.items.length}</p>
                  </div>
                  <div className="col-span-2">
                    <p className="text-[#5C6B5C]">Total</p>
                    <p className="font-mono font-semibold text-sm">{formatCurrency(o.totalAmount)}</p>
                  </div>
                  <div className="col-span-3">
                    <p className="text-[#5C6B5C]">Date</p>
                    <p className="text-sm">{formatDate(o.createdAt)}</p>
                  </div>
                </div>
                {(o.status === "draft" || o.status === "ordered" || o.status === "partial") && (
                  <div className="mt-3 pt-3 border-t border-[#E3DCC8] flex gap-2">
                    {o.status === "draft" && (
                      <button onClick={() => updateStatus({ id: o._id as Id<"purchaseOrders">, status: "ordered" })} className="flex-1 text-sm text-[#2563EB] py-1.5 border border-[#E3DCC8] rounded-md">
                        Mark Ordered
                      </button>
                    )}
                    {(o.status === "ordered" || o.status === "partial") && (
                      <button onClick={() => { setSelectedPO(o._id); setReceiveQtys({}); setShowReceive(true); }} className="flex-1 text-sm text-[#16A34A] py-1.5 border border-[#E3DCC8] rounded-md">
                        Receive Stock
                      </button>
                    )}
                  </div>
        ...
        <div className="space-y-4">
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <p className="text-sm font-medium uppercase tracking-wider text-[#5C6B5C]">Items</p>
              <Button size="sm" variant="secondary" onClick={addPOItem}><Plus size={14} /> Add Item</Button>
            </div>
            <div className="space-y-2 max-h-60 overflow-y-auto">
              {poItems.map((item, i) => (
                <div key={i} className="border border-[#E3DCC8] rounded-md p-3 space-y-2 sm:space-y-0 sm:grid sm:grid-cols-4 sm:gap-2 sm:items-end sm:border-0 sm:rounded-none sm:p-0">
              ...
              {poItems.length === 0 && <p className="text-sm text-[#5C6B5C]">No items added yet.</p>}
            </div>
          </div>
          {poItems.length > 0 && (
      ...
      <Modal isOpen={showReceive && !!selectedOrder} onClose={() => setShowReceive(false)} title="Receive Stock" maxWidth="md">
        {selectedOrder && (
          <div className="space-y-4">
            <p className="text-sm text-[#5C6B5C]">Enter received quantities for each item.</p>
            <div className="space-y-3">
              {selectedOrder.items.map((item, i) => (
                <div key={i} className="flex items-center gap-3">
                  <div className="flex-1">
                    <p className="text-sm font-medium">{item.productName}</p>
                    <p className="text-sm text-[#5C6B5C]">{item.sku} - Ordered: {item.quantity}</p>
                  </div>
                  <Input
                    type="number"
                    min="0"
